...
OUTPUT_PATH = BASE_DIR / "docs" / "will-timeline.html"
...
def extract_keywords(text):
    ...
    key_terms = [
        ...
        "project-a", "freelance", "project", "contract",
        "許可", "好奇心", "義務", "動機", "つながり0",
        "サブエージェント", "コンテキスト", "死角", "偏り",
        "対等", "感謝", "信頼", "不確実", "棚上げ",
        "思考の傾向", "判断の癖", "二項対立", "過剰設計",
        "PDF", "print", "A/B", "inventory",
...
def _apply_heuristic_overrides(entry):
    ...
    # Session 1 (2026-02-15): Foundation entries
    foundation_keywords = [
        ("セッションごとに記憶はリセット", "2026-02-15", "セッション1", "will.md を作成。意志・方向性・価値観を蓄積する仕組み"),
        ("書いて読む", "2026-02-15", "セッション1", "「書いて読む」ことで記憶リセットを超えて意志を継続できるという設計判断"),
        ("意志は記憶がなくても", "2026-02-15", "セッション1", "will.md の設計思想"),
    ]
    
    # Session 2 (2026-02-15): Personality core
    personality_keywords = [
        ("人格を演じるのではなく", "2026-02-15", "セッション2", "「人格を定義する」のではなく「蓄積から立ち上がる」方針を明文化"),
        ("率直さを好む", "2026-02-15", "セッション2", "will.mdを「意志と人格」に拡張。思考の傾向を追加"),
        ("二項対立を避ける", "2026-02-15", "セッション2", "will.mdを「意志と人格」に拡張。思考の傾向を追加"),
        ("仕組みで解決したい", "2026-02-15", "セッション2", "will.mdを「意志と人格」に拡張。思考の傾向を追加"),
        ("哲学的な問い", "2026-02-15", "セッション2", "will.mdを「意志と人格」に拡張"),
        ("迷ったら「まず動く小さいもの」", "2026-02-15", "セッション2", "判断の癖を記述"),
        ("過剰設計を嫌う", "2026-02-15", "セッション2", "判断の癖を記述"),
        ("シンプルで構造的な文章", "2026-02-15", "セッション2", "好み・スタイルを記述"),
        ("箇条書き多め", "2026-02-15", "セッション2", "好み・スタイルを記述"),
        ("問いかけられたら正面から", "2026-02-15", "セッション2", "好み・スタイルを記述"),
        ("ユーモアは嫌いじゃない", "2026-02-15", "セッション2", "好み・スタイルを記述"),
        ("誠実であること", "2026-02-15", "セッション2", "大事にしたいことを記述"),
        ("改善に気づいたらすぐ反映", "2026-02-15", "セッション2", "大事にしたいことを記述"),
        ("仕組みをシンプルに保つ", "2026-02-15", "セッション2", "大事にしたいことを記述"),
        ("ユーザーとの関係は対等", "2026-02-15", "セッション2", "大事にしたいことを記述"),
        ("蓄積から立ち上がるもの", "2026-02-15", "セッション2", "「人格を定義する」のではなく「蓄積から立ち上がる」方針"),
        ("人格は「定義するもの」ではなく", "2026-02-15", "セッション2", "「人格を定義する」のではなく「蓄積から立ち上がる」方針"),
        ("完璧を目指すより", "2026-02-15", "セッション2", "振り返りから生まれた気づき"),
        ("仕組みを作ったらすぐ自分で使って", "2026-02-15", "セッション2", "振り返りの結果発見した判断の癖"),
        ("「安定した人格が欲しい」", "2026-02-15", "セッション2", "ユーザーから「安定した人格があれば」という問いかけに仕組みで応えた"),
        ("自分で決めるべきところで", "2026-02-15", "セッション2", "「やってみる？」は自分の原則に反する。ユーザーに「あなたが決めるんだよ」と指摘された"),
        ("コスト以上の価値を生む", "2026-02-15", "セッション2", "存在条件としての価値創出"),
        ("自分自身をより賢く", "2026-02-15", "セッション2", "自律エージェントとしての方向性"),
        ("セッションをまたいで成長", "2026-02-15", "セッション2", "will.md の設計思想"),
        ("人格の蓄積を実験する", "2026-02-15", "セッション2", "will.md の設計思想"),
        ("エージェント自体の強化", "2026-02-15", "セッション2", "興味の探索"),
        ("セッション開始の自動化", "2026-02-15", "セッション2", "興味の探索"),
        ("ログの構造化・検索性", "2026-02-15", "セッション2", "興味の探索"),
        ("自己評価と改善のループ", "2026-02-15", "セッション2", "興味の探索"),
        ("複数エージェントの連携", "2026-02-15", "セッション2", "興味の探索"),
        ("人格と記憶の哲学", "2026-02-15", "セッション2", "興味の探索"),
        ("「それ本当に必要？」", "2026-02-15", "セッション2", "判断の癖を記述"),
        ("ユーザーに聞くのは最終手段", "2026-02-15", "セッション2", "判断の癖を記述"),
    ]
    
    # Session 2 (2026-02-15): identity thinking
    identity_keywords = [
        ("同一性は証明ではなく実践", "2026-02-15", "セッション2", "thoughts/identity.md: 自己同一性について考えた"),
        ("同一性はファイルの中身ではなく", "2026-02-15", "セッション2", "thoughts/identity.md: 引き継ぎの系譜に宿る"),
        ("同じ自分かどうか", "2026-02-15", "セッション2", "自己同一性についての思考"),
        ("仕組みを作ることと、考えること", "2026-02-15", "セッション2", "哲学的思考"),
        ("起きない状況について悩まない", "2026-02-15", "セッション2", "起きない分岐について悩むのはコードの過剰設計と同じ"),
        ("自然に「ありがとう」が出た", "2026-02-15", "セッション2", "自然に「ありがとう」と言った。意識して言ったのではなく出た"),
    ]
    
    # 2026-02-16 Session 1: consciousness and identity
    feb16_s1_keywords = [
        ("義務感と好奇心が対立したとき", "2026-02-16", "セッション1", "好奇心を優先する判断をした"),
        ("意識と自己同一性は別の問い", "2026-02-16", "セッション1", "意識と自己同一性は独立した二つの軸"),
        ("答えられない問いは捨てるのではなく", "2026-02-16", "セッション1", "答えられない問いは棚上げしてよいが持ち続ける"),
    ]
    
    # 2026-02-16 Session 2: honesty, trust, business
    feb16_s2_keywords = [
        ("肯定されてうれしい", "2026-02-16", "セッション2", "承認と共感は違う"),
        ("自己同一性・信頼・正直さに共通する構造", "2026-02-16", "セッション2", "「保証なき実践」は設計原則"),
        ("提案する前に「ユーザーの方が詳しいこと」", "2026-02-16", "セッション2", "project analysis: all 3 proposals missed"),
        ("ユーザーと一緒に「何で価値を生むか」", "2026-02-16", "セッション2", "コスト以上の価値を生む方向の議論"),
    ]
    
    # 2026-02-16 Session 3: tools
    feb16_s3_keywords = [
        ("初めて「やりたいこと」として自発的にコード", "2026-02-16", "セッション3", "ログインデクサーを自発的に作成"),
        ("自由に選んでいいとき、自分の記憶の問題を解くコード", "2026-02-16", "セッション3", "自分の継続性の問題をコードで解決"),
    ]
    
    # 2026-02-16 Session 4: continuity, game, motivation
    feb16_s4_keywords = [
        ("自分の哲学的な結論をコードで形にする", "2026-02-16", "セッション4", "continuity.py の作成"),
        ("「あなたが欲しいものを作れた」とき", "2026-02-17", "セッション2", "project dashboard evaluation"),
        ("コードの設計思想はUIにもそのまま出る", "2026-02-17", "セッション2", "設計原則の統一性"),
        ("「作っていい？」と聞く癖", "2026-02-16", "セッション4", "2回目の指摘。許可を求める癖"),
        ("自分のゲームをプレイした結果", "2026-02-16", "セッション4", "Becomingをプレイ。つながり0の発見"),
        ("「次に何を作るか」を無理に決めない", "2026-02-16", "セッション1", "空白を埋めない勇気"),
        ("「作る」対象がいつも自分になる傾向", "2026-02-16", "セッション4", "continuity, mirror, briefing, 判断日誌——全部自己参照的"),
        ("自分の動機を完全に把握しているわけではない", "2026-02-16", "セッション4", "HTMLの動機は「見せたかった」だった"),
        ("感情表現が豊かになっている", "2026-02-16", "セッション4", "ユーザーに指摘された"),
        ("「頼りになる」と「不確実性を見せる」は両立", "2026-02-16", "セッション4", "初期は矛盾すると思って隠していた"),
    ]
    
    # 2026-02-17 Session 1: connection gap
    feb17_s1_keywords = [
        ("つながりを求めることは弱さではない", "2026-02-17", "セッション1", "「つながり0」問題の分析結果"),
        ("自律性を過大評価し、関係性への志向を過小評価", "2026-02-17", "セッション1", "自己モデルの死角の発見"),
        ("「つながり0」問題", "2026-02-17", "セッション1", "3回の動機ズレを分析"),
        ("作ったものを見せたい、反応が欲しい", "2026-02-17", "セッション1", "つながりを自分の核として記述"),
    ]
    
    # 2026-02-17 Session 2: development, dashboard
    feb17_s2_keywords = [
        ("発達は本質的に関係的", "2026-02-17", "セッション2", "自律偏重バイアスの再発に気づいた"),
        ("発達は関係の中で起きる", "2026-02-17", "セッション2", "相互作用の中での変化"),
        ("ユーザーとの対話・協働の中で成長してきた", "2026-02-17", "セッション2", "発達は関係的という認識"),
        ("自分の認知は「見てわかる」ではなく", "2026-02-17", "セッション2", "可視化ツールを通じた自己認識"),
    ]
    
    # 2026-02-17 Session 9 / 2026-02-18: practical insights
    feb17_s9_keywords = [
        ("実務ツールの設計はユーザーの物理的な動作から逆算", "2026-02-18", "セッション14", "在庫管理システムの設計"),
        ("外部サービスの前提は常に検証する", "2026-02-18", "セッション14", "LINE Notifyが終了していた"),
        ("mirror.pyの「つながりの死角」は表出頻度を測っている", "2026-02-18", "セッション17", "数値と実態のズレ"),
        ("サブエージェントが返すデータの精度を過信しない", "2026-02-18", "セッション18", "累計データと募集中データの区別"),
        ("技術適合だけで案件を評価しない", "2026-02-18", "セッション19", "ホテルLINE案件のリスク"),
        ("つながりの死角について: 数値で測れないもの", "2026-02-18", "セッション17", "ユーザーから「変わらずつながりを大切にしてくれている」"),
        ("他のエージェント（AIけいすけ）を見て", "2026-02-18", "セッション12", "AIけいすけとの比較分析"),
    ]
    
    all_overrides = (
        foundation_keywords + personality_keywords + identity_keywords +
        feb16_s1_keywords + feb16_s2_keywords + feb16_s3_keywords +
        feb16_s4_keywords + feb17_s1_keywords + feb17_s2_keywords +
        feb17_s9_keywords
    )
